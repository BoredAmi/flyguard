...
**[> Watch one run](https://claude.ai/code/artifact/cde78149-e2da-4dd9-b002-dffdc445b054)** --
corridor map, the agent's own eye view, and the two hemispheres' LPLC2 rates on one
timeline. The recorder drives `avoid.run_trial` through a frame-capture hook rather than
reimplementing the loop, and it reproduces the benchmark trial exactly.
...
10 arenas, 7 obstacles each, nothing trained:
...
[CLAUDE.md](CLAUDE.md) has the numbers:
...
**[> Interactive recording of a live run](https://claude.ai/code/artifact/c57f2f77-4815-40fd-ad81-64f6bd7a41bd)** --
rendered scene, real spike raster, and `/cmd_vel` on one synchronized timeline. Every dot in
the raster is a real LIF spike from a real sampled neuron.
([Hemisphere sweep chart](https://claude.ai/code/artifact/ff2f4291-3283-44de-998b-0a76ef7131ba) too.)
...
PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 pytest -q      # 206 tests
```
...
- **MuJoCo's EGL context and PyTorch's CUDA init in one interpreter can abort the process**
  ...
  PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 MUJOCO_GL=egl pytest -q \
      --ignore=tests/test_baseline_cnn.py          # 199 passed
...
- **The direction convention is assumed, not validated -- and it's load-bearing.**
  ...
  and I can't presently distinguish those.
...
tests/                      206 tests; runtime + calibration need no MuJoCo and no CSVs
```
